...
/* ── Scroll Reveal (IntersectionObserver) — synchronisé par ligne ─── */
function reveal() {
    var elements = Array.from(document.querySelectorAll('.rv:not(.show)'));
    if (!elements.length) return;

    /* Regroupe les éléments dont le bord supérieur est identique (±4px) */
    function getRow(el) {
        return Math.round(el.getBoundingClientRect().top / 4) * 4;
    }

    var observer = new IntersectionObserver(function(entries) {
        /* Collecte toutes les entrées visibles à ce tick */
        var visible = entries.filter(function(e) { return e.isIntersecting; });
        if (!visible.length) return;

        /* Regroupe les éléments visibles par ligne (même offsetTop arrondi) */
        var rows = {};
        visible.forEach(function(entry) {
            var rowKey = Math.round(entry.target.offsetTop / 6) * 6;
            if (!rows[rowKey]) rows[rowKey] = [];
            rows[rowKey].push(entry.target);
            observer.unobserve(entry.target);
        });

        /* Déclenche chaque ligne avec un micro-délai en cascade (0 ms entre éléments
           de la même ligne, 60 ms entre lignes successives) */
        var rowKeys = Object.keys(rows).sort(function(a, b) { return a - b; });
        rowKeys.forEach(function(key, rowIndex) {
            rows[key].forEach(function(el, colIndex) {
                /* Petit décalage en éventail entre colonnes pour un effet naturel */
                el.style.transitionDelay = (rowIndex * 60 + colIndex * 40) + 'ms';
                el.classList.add('show');
                /* Nettoie le délai après l'animation pour ne pas bloquer les re-triggers */
